...
export default {
  ...
  computed: {
    count () {
      return this.$store.getters.banner.getCount
    },
    colorHover () {
      let color = this.$store.state.bannerhovercolor1
      return `<style>
      .ul2>li:hover a{
        color:${color}
      }
  ...
  components:{
    'my-component':{
      ...
      data:function(){
        return {
          c1:this.$store.getters.banner.getCount
        }
      },
      computed:{
        ee(){
  ...
  methods:{
    click1:function(index){
      ...
      console.log("this.$store.state.bannercount: "+this.$store.state.bannercount)
    }
  },
  beforeMount(){
  ...
  mounted:function(){
    this.list = this.$store.state.bannertodos
